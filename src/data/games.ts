...
export interface GamePostMortemSection {
  type:
    | 'heading'
    | 'paragraph'
    | 'list'
...
export interface GamePostMortem {
  title: string;
  subtitle?: string;
  description: string;
  publishDate: string;
  ...
  tags?: string[];
  categories?: string[];
  sections: GamePostMortemSection[];
}
...
export interface Game {
  ...
  year?: number; // Year the game was created/submitted
  postMortem: GamePostMortem;
}
...
export const games: Game[] = [
  {
    ...
    description:
      'A strategic resource management game featuring a curious cat collecting bioluminescent fireflies in the starry night sky. Players must balance exploration, resource gathering, and strategic timing while navigating an atmospheric world filled with glowing creatures. Built entirely in vanilla JavaScript using HTML5 Canvas within the 13KB file size limit.',
    image: '/games/nyx-felis.png',
    ...
    postMortem: {
      ...
      sections: [
        ...
        },
        {
          type: 'pull-quote',
          content:
            '"An animal\'s eyes have the power to speak a great language." - David Attenborough',
        },
